				...
				for (var i = 0; i < elements.length; i++) {
					if (elements[i].tagName == "IFRAME") {
						if (elements[i].contentWindow && elements[i].contentWindow.document) {
							elements[i].classList.add("optanon-not-active");// Add a class so loading icon is gone.
...
const loadPageData = ({ request }) => {
	...
	return fetch(`${config.api_url}getmetadata?url=${encodeURIComponent(meta_path + query)}&locale=${config.locale}`, options)
		.then(resp => resp.json())
		.then(meta => {
					...
					if (test_pathname.toLowerCase() == redirecto.toLowerCase()) {
						...
						return new Response("", {
							...
							headers: {
								Location: redirecto,
								replace: true,
								reload: meta.redirect_reload || false,
							},
						});
					}
					else {
	...
	useEffect(() => {
		...
		data.page?.then((result) => {
			if (result.status === 302) {
				// Headers are strings.
				if (result.headers.get("reload") === 'true') {
