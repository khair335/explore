				...
				<div className="carousel-indicators-text">
					<ol className="carousel-indicators">
						{props?.content_blocks.map((content_block, index) => (
							<li tabIndex="0" className={classnames({ active: props.activeIndex === index })} key={content_block.content_id} onClick={() => props.goToIndex(index)} onKeyDown={(e) => props.handleKeyDown(e, index)}>{content_block.tab_title}</li>
						))}
					</ol>
				</div>
			);
			...
			return (
				<div className="carousel-indicators-image">
					<ol className="carousel-indicators">
						...
						{props?.content_blocks.map((content_block, index) => (
							<li tabIndex="0" className={classnames({ active: props.activeIndex === index })} key={content_block.content_id} onClick={() => props.goToIndex(index)} onKeyDown={(e) => props.handleKeyDown(e, index)}>
	...
	return (
		...
		<Row>
			...
			<Col>
				...
				>
					<Indicators content_blocks={content_blocks} activeIndex={activeIndex} goToIndex={goToIndex} handleKeyDown={handleKeyDown} indicator_type={indicator_type} />
					{slides}
					{show_controls &&
						<>
							<CarouselControl
