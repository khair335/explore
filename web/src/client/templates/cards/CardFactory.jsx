                ...
                <div className="card-footer"><ul className="cb-cta-link">
                    {this.props.data.links && this.props.data.links.map((link, index) =>
                        <li key={link.content_id + index}>
                            <SiteLink aria-label={link.title} to={link.url} target={link.target} subtype={link.subtype} media_id={link.media_id} className="card-link" key={link.content_id || index}>{link.title}</SiteLink>
                        </li>
                    )}
                </ul>
                </div>
                            ...
                            {this.props.data.links &&
                                <div className="card-footer">
                                    <ul className="cb-cta-link">
                                        {this.props.data.links && this.props.data.links.map((link, index) =>
                                            <li key={link.content_id + index}>
                                                <SiteLink aria-label={link.title} to={link.url} target={link.target} subtype={link.subtype} className="card-link" key={link.content_id || index}>{link.title}</SiteLink>
                                            </li>
                                        )}
                                    </ul>
                                </div>
...
export const VideoCard = (props) => {
    ...
    const id = video_content?.media_id || video_content?.video_id || video_content?.id
    ...
    let data = {
        ...
        url: url_path,
        id: id,
        image: video_content?.images?.poster || video_content?.images,
            image_src: video_content?.thumbnail || video_content?.poster || video_content?.images,
        // image: video_content?.image_src?.poster?.src || video_content?.images?.poster || video_content?.images,   // Object
        // image_src: video_content?.image_src?.poster?.src || video_content?.thumbnail || video_content?.poster || video_content?.images,       // Url
    ...
    if (!video_content?.where_the_video_should_be_hosted_) { // Could be broadcom or vmware, so check if this exists
        data = {
        ...
        <div className="VideoCard card">
            <div className={`card-body ${display_horizontal ? 'card-body-related-videos' : ''}`}>
                <SiteLink to={data.url} target={target} rel="noopener noreferrer" className={`video-thumbnail-link ${display_horizontal ? 'col-related-videos' : ''}`}>
                    {!video_content?.images?.poster ? <ImageBase src={data.image || data.image_src} alt={data?.title} className="video-thumbnail" /> :
                        <ImageBase image={data.image || data.image_src} alt={data?.title} className="video-thumbnail" />}
