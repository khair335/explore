							...
							this.props.data.contacts.map(contact => {
								return (
									<address>
										...
										{contact.email && <>{localizeText("CY12","Email:")} <a className="lnk" href={"mailto:"+contact.email} target="_blank">{contact.email}</a> <br/></>}
										{contact.phone && <>{localizeText("CY13","Telephone:")} {contact.phone}</>}
									</address>		
								)
							})
							:
