								...
								{showData?.map((item, index) => (
									<tr key={`${item.content_id}-${index}`}>
										{keysToDisplay.filter(key => visibleColumns.has(key)).map((key, index) => {
											...
											if (dataKey === 'submitter') {
												return <td className={className} key={`${item.content_id}-${key}`}><SiteLink to={item[dataKey]?.url} target='__blank'><img src={item[dataKey].logo?.src} alt={item[dataKey].logo?.alt} /></SiteLink></td>
											}
