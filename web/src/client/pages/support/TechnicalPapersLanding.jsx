...
const TechnicalPapersLanding = (props) => {
    ...
    const filters = [
        { label: "Product", filter_attribute: 'Product', attribute: "products", tags: products },
        { label: "Publisher", filter_attribute:'Publisher', attribute: "publisher", tags: publisher },
        { label: "Area", filter_attribute:'Topic', attribute: "area", tags: area },
    ];

    const filterLabels = props.data.filter_attributes?.map(attr => Object.values(attr)[0]);

    const product_json = {};
    const publisher_json = {};
    const area_json = {};
    const keysToDisplay = ["Title", "Date", "Document"];
        ...
        <div id="TechnicalPapersLanding">
            <div className='technical-papers-top-search-container'>
                ...
                <div className='container'>
                    ...
                    <div>
                        <MultiSelectFilter
                            items={filters?.filter(f => filterLabels?.includes(f.filter_attribute))}
                                ...
                                {showData?.map(item => (
                                    <>
                                        <tr key={item.content_id} onClick={() => toggleRow(item.content_id)}>
                                            ...
                                            {item?.filter_values?.publisher?.length > 0 && <td>{item.filter_values.publisher.join(', ')}</td>}
                                            {item?.revision_date && <td>{displayDate(item.revision_date)}</td>}
