...
class VideoPage extends PageComponent {
	constructor(props) {
		...
		this.state = {
			...
			base_url: `https://api.swiftype.com/api/v1/public/engines/search.json?engine_key=FGq2x_7zpGpZ68xHePFN&document_types[]=videos&filters[videos][vod_on_demand_publish][]=True&filters[videos][vod_on_demand_publish][]=&filters[videos][complete]=true&filters[videos][state]=ACTIVE`,
			copy_tooltip: '',
			copySuccess: '',
			activeTab: 'details',
			relLoading: false,
		...
		this.setError = this.setError.bind(this); // Added to bind setError
	}
	...
	setMediaData(data) {
		const searchUrl = this.generateSearchString(data?.customFields) || this.formatSearchQuery(data?.title);
		const account = data?.customFields?.where_the_video_should_be_hosted_ === "VMware" ? 'vmware' : 'explore';
		const finalUrl = `${this.state.base_url}&${searchUrl}&filters[videos][account]=${account}`;
	...
	generateSearchString(customFields) {
		if (Object.keys(customFields).length === 1 && customFields.hasOwnProperty('where_the_video_should_be_hosted_')) {
			return '';
		}

		// Initialize an array to store all filter values
		const allValues = [];

		Object.entries(customFields)
			.filter(([key]) => !key.startsWith('speaker')) // Filter out keys that start with 'speaker'
			.forEach(([key, value]) => {
				// Split the values and trim whitespace
				const values = value.split(' | ').map(val => val.trim());

				// Add the encoded values to the allValues array
				allValues.push(...values.map(val => encodeURIComponent(val)));
			});

		// Convert the allValues array to the desired search string format
		const searchString = `q=${allValues.join(',')}`;

		return searchString;
	}

	formatSearchQuery(query) {
		// Replace special characters with spaces and split into keywords
		const keywords = query.replace(/[-–—/\\^$*+?.()|[\]{}]/g, ' ').split(/\s+/);
		return `q=${encodeURIComponent(keywords.map(encodeURIComponent).join(' '))}`;
	}
		...
		return (
			<>
				<div className="share-section">
					<ul>
						{	// Wrap in UL for accessibility
							social_links.map((social_link) => (

								<li key={social_link.name}>
									<SiteLink aria-label={social_link.name} to={social_link.socialLink} target='_blank'><span className={social_link.className}><span className='path1'></span></span></SiteLink>
								</li>
							))
						}
					</ul>
	...
	render() {
		...
		return (
			<Container id="Video">
				<Loading isLoading={this.state.loading} className="video-loading">
					{!this.state.error
						? <div className="video-content">
							...
							<Row>
								<div className={classnames("video-box col-lg-12 col-md-12 col-sm-12 col-xs-12")}>
									<Video mediaid={this.state.mediaid} controls onMediaData={this.setMediaData} onError={this.setError} {...settings} />
