...
export async function getBrightcoveRelatedVideos(search_url, videoID) {
    return fetch(`${search_url}`, {
        ...
        .then(resp => resp.json())
        .then(json => {
            let videos = json.records?.videos?.map(video => {
                ...
                if (video?.where_the_video_should_be_hosted_ == undefined) {
                ...
                // set thumbnail
                const thumbnail = video?.images || video?.images?.poster?.src; // use larger image (poster) as thumbnail - RH
                // set isVMWare bool value
                const isVMWare = (video?.where_the_video_should_be_hosted_ == undefined)? false : true;
