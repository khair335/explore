...
export function fetchPage(path, init) {
	...
	return fetch(`${config.api_url}getmetadata?url=${path}&locale=${config.locale}`, options)
		.then(resp => resp.json())
		.then(meta => {
			if (meta.updateddate) {
 ...
 */
	getTranslations() {
		...
		fetch(`${config.api_url}label/translations`, { credentials: config.api_credentials })
			.then(resp => resp.json())
			.then(json => {
	...
	/**
	 *  @brief 
	 *  @details Is the element in view
	 */
