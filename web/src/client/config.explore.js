/**
 *  @file config.explore.js
 *  @brief Config for Explore
 */
...
class Config {
	constructor() {
		...
		this.video = {
			...
			videoLibraryPath: '/explore/video-library',
		};
		...
		// Unique to explore, we will use what site we are on based on path name
		// Us or eu
		this.explore_event = '';
		if (location.pathname?.includes('/us')) {
			this.explore_event = 'us';
		}
		else if (location.pathname?.includes('/eu')) {
			this.explore_event = 'eu';
		}
		...
		this.hide_breadcrumbs = true;
		
		if (ENVIRONMENT) {
			switch (ENVIRONMENT) {
				case 'local':
					this.swifttype.engine_key = 'J3yan3XpFywGvRxQMcEr'; // 'myuc5Dc4M6MBzSkEzkxN'; //
				...
				case 'development':
					...
					this.swifttype.engine_key = '_Ce4v9fcydUUcrhG1vXj';
					this.locale_base = {
		...
		// Event filter
		let filter_event = '';
		if (this.explore_event) {
			filter_event = `&filters[pages][explore_event]=${this.explore_event}`;
		}

		this.site_search = {
			get endpoint() {

				return `https://api.swiftype.com/api/v1/public/engines/search.json?engine_key=${self.swifttype.engine_key}&document_types[]=pages&facets[pages][]=content_type&filters[pages][locale]=${self.locale}${show_china}${filter_event}`;
			},

			get typeahead_endpoint() {
				return `https://api.swiftype.com/api/v1/public/engines/suggest.json?engine_key=${self.swifttype.engine_key}&document_types[]=pages&filters[pages][locale]=${self.locale}&document_types[]=pages${show_china}${filter_event}`;
			},

			get search_url() {
				return `/explore/${self.explore_event}/search`;
			}
		};
		...
		this.video_search = {
			endpoint: `https://api.swiftype.com/api/v1/public/engines/search.json?engine_key=${this.swifttype.engine_key}&document_types[]=videos&&filters[videos][locale]=${this.locale}${show_china}&filters[videos][vod_on_demand_publish][]=!False&filters[videos][complete]=true&filters[videos][state]=ACTIVE&facets[videos][]=products&facets[videos][]=sessiontype&facets[videos][]=audience&facets[videos][]=track&facets[videos][]=level&filters[videos][year][]=!&filters[videos][account]=explore`,
