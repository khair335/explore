...
const USER_AGENT = 'brcm-ui';		// Use custom user agent for this request.
...
app.get('/justlayout', (req, res) => {
	...
	// 248144 Use fixed user agent vs req.headers['user-agent']
	request({ url: `${protocol}://${BASIC_AUTH}${req.headers.host}/api/getmetadata?url=&locale=${locale}`, headers: { 'user-agent': USER_AGENT }, json: true }, (err, response, data) => {
			...
			request({
				...
				(err, response, data) => {
					...
					if (data) {
						...
						request({ url: `${protocol}://${BASIC_AUTH}${req.headers.host}/api/label/translations`, headers: { 'user-agent': req.headers['user-agent'] }, json: true }, (err, response, translations) => {
							if (err) { return console.log(err); }

							// Validate translations is a json
		...
		request({
			...
			(err, response, data) => {
				...
				if (data) {
					...
					// 248144 Use fixed user agent vs req.headers['user-agent']
					request({ url: `${protocol}://${BASIC_AUTH}${req.headers.host}/api/label/translations`, headers: { 'user-agent': USER_AGENT }, json: true }, (err, response, translations) => {
						if (err) { return console.log(err); }
...
app.use(async (req, res, next) => {
	...
	if (path.dirname(req.path) === "/" && path.extname(req.path).length > 0) {
		...
		try {
			...
			console.log("-----------------------");
			console.log("metaurl:", `${protocol}://${BASIC_AUTH}${req.headers.host}/api/getmetadata?url=${path}&locale=${locale}`);
			...
			let response = await fetch(`${protocol}://${req.headers.host}/api/getmetadata?url=${path}&locale=${locale}`, options);
			...
			if (head.status === 301) {
				...
				const urls = [{
					url: `${protocol}://${req.headers.host}/api/label/translations`, // BCCS5-6: Fetch our translations and throw in our html.
					options: options,
				}];
				// }, {
				...
				const texts = responses.map((response) => response.text());
				let data = await Promise.all(texts);

				// Make sure its a valid JSON and not junk.
				data = data.map((text) => {
					try {
